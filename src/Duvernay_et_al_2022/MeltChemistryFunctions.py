...
from ChemistryData import part_coeff, mnrl_mode_coeff, radii, valence
...
def run_integrator(part_arr, ode_dX, cs_old):
    ...
    def ode_rhs(X, cs):
        if mnrl_outputs.get(X) is None:  # X-value not yet encountered
            Fn, pn, Dn = mineralogy(
                X, X_0, Fn_0, mask_ode[mask_step], D[mask_step],
    ...
    Fn_0, melting_step = solid_composition(X_0, P_0, mnrl_mode_coeff, eNd), 1
    ...
    extra_args = (mnrl_mode_coeff, eNd, part_arr)
    ...
    cl[mask_ode] = cs[mask_ode] * (1 - X) / (
        D_bulk * (1 - X_0) - P_bulk * (X - X_0))[mask_step]
