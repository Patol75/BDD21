...
@cc.export('mineralogy', 'Tuple((f8[:], f8[:], f8[:, :]))'
           '(f8, f8, f8[:], f8[:], f8[:, :], f8[:, :], u8[:], '
           'DictType(unicode_type, f8[:]), f8, DictType(unicode_type, f8[:]))')
def mineralogy(X, X_0, Fn_0, pn, D, ri, val, quad_poly_coeff, eNd, part_arr):
    ...
    # Proportions of phases remaining in the residual solid
    Fn = solid_composition(X, P, quad_poly_coeff, eNd)

    # Proportions of phases entering the melt
    if X > X_0:
        pn = (Fn_0 * (1 - X_0) - Fn * (1 - X)) / (X - X_0)

    # Partition coefficients between all elements and mineral phases considered
    Dn = partition_coefficient(P, T, X, D, ri, val, Fn)
...
@cc.export('solid_composition', 'f8[:]'
...
def solid_composition(X, P, quad_poly_coeff, eNd):
    ...
    Fn[0], Fn[2] = mnrl_prop["ol"], mnrl_prop["cpx"]
    Fn[3], Fn[4], Fn[5] = mnrl_prop["plg"], mnrl_prop["spl"], mnrl_prop["gnt"]

    # Incorporate orthopyroxene and adjust mineral proportions (sum to 1)
    sum_ol_cpx_al = Fn.sum()
    if sum_ol_cpx_al <= 1:  # Remaining proportion -> orthopyroxene
        Fn[1] = 1 - sum_ol_cpx_al
...
@cc.export('partition_coefficient',
           'f8[:, :](f8, f8, f8, f8[:, :], f8[:, :], u8[:], f8[:])')
@njit('f8[:, :](f8, f8, f8, f8[:, :], f8[:, :], u8[:], f8[:])')
def partition_coefficient(P, T, X, D, ri, val, Fn):
    ...
    # # # # # #
    if mask_30.any():
        F_Al = al_phase_select(Fn, 1.56e-3, 5.64e-3)
    ...
    if mask_11.any() or mask_21.any() or mask_31.any() or mask_41.any():
        ...
        X_Mg_Mel = al_phase_select(Fn, 0.14 * X + 0.722, 0.207 * X + 0.701)
