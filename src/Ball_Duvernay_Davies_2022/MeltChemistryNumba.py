...
def mineralogy(
    ...
    Fn_spl, Fn_gnt = solid_composition(X, P, mnrl_mode_coeff, eNd)
...
@cc.export(
    "solid_composition",
    "Tuple((f8[:], f8[:]))(f8, f8, DictType(unicode_type, f8[:, :]), f8)",
)
...
def solid_composition(X, P, mnrl_mode_coeff, eNd):
    ...
    modal_ab = {}
    # Order: [ol, opx, cpx, plg, spl, gnt]
    Fn_spl, Fn_gnt = np.zeros(6), np.zeros(6)
    pres_poly_var = np.asarray([P**2, P, 1])
    melt_poly_var = np.asarray([X, 1])
    ...
    Fn_spl.clip(0, 1, Fn_spl)
    Fn_gnt.clip(0, 1, Fn_gnt)

    for Fn_al in (Fn_spl, Fn_gnt):
        # Deduce orthopyroxene mode and ensure modal abundances sum to 1
        sum_modes_ol_cpx_al = Fn_al.sum()
        if sum_modes_ol_cpx_al <= 1 - X:  # Remaining mode -> orthopyroxene
            Fn_al[1] = 1 - X - sum_modes_ol_cpx_al
            Fn_al /= 1 - X
        else:  # Enforce proportions sum to 1
            Fn_al /= sum_modes_ol_cpx_al

        # Account for systematic differences between primitive and depleted
        # modal abundances using the correction (Figure 3) of
...
@cc.export(
    ...
    "f8[:, :](f8, f8, f8, u8[:], f8[:, :], f8[:, :], u8[:], f8, f8)",
)
@njit("f8[:, :](f8, f8, f8, u8[:], f8[:, :], f8[:, :], u8[:], f8, f8)")
def partition_coefficient(P, T, X, ele_ind, D, ri, val, X_spl_in, X_gnt_out):
    ...
    if mask_31_const.any():
        ...
        X_Al = al_phase_select(X, X_spl_in, X_gnt_out, 1.56e-3, 5.64e-3)
        # Forsterite content in olivine | Magnesium number
        Mg_num = al_phase_select(
            X, X_spl_in, X_gnt_out, 0.059 * X + 0.904, 0.07 * X + 0.897
        )
    ...
    if mask_21.any() or mask_31_const.any():
        ...
        #     X, X_spl_in, X_gnt_out, -0.014 * X + 0.012, -0.047 * X + 0.027
        # )
