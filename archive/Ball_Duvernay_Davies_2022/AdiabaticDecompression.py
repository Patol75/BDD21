...
X_spl_in, X_gnt_out = calc_X_spl_in_gnt_out(
    1, 1, depth[0], depth[-1], part_arr, adiab_grad / rho_mantle / g * 1e9, rho_mantle
)
...
Fn_0_spl, Fn_0_gnt = solid_composition(0, part_arr["pressure"][0], mnrl_mode_coeff, eNd)
Fn_store[0] = al_phase_select_array(0, X_spl_in, X_gnt_out, Fn_0_spl, Fn_0_gnt)
...
for i in range(sum(mask) - 1):
    ...
    ode_dX, Fn, cs, cl = run_integrator(part_arr, ode_dX, X_spl_in, X_gnt_out, cs)
    # Store results
