...
def run_integrator(
    part_arr, ode_dX, cs_old, cs_0, elements, ele_ind_map, const_ele_ind, src_depletion
):
    ...
    def ode_rhs(X, cs):
        if mnrl_outputs.get(X) is None:  # X-value not yet encountered
            ...
            D_bulk, P_bulk = ones_like(cs), ones_like(cs)
            D_bulk[mask_step], P_bulk[mask_step] = Dn @ Fn, Dn @ pn

            Dn_full = zeros((cs.size, 6))
            Dn_full[mask_step] = Dn

            mnrl_outputs[X] = [Fn, pn, Dn_full, D_bulk, P_bulk]
    ...
    mask_step = ones_like(mask_ode, dtype=bool)

    X_0, P_0 = part_arr["melt_fraction"][0], part_arr["pressure"][0]
    Fn_0, melting_step = solid_composition(X_0, P_0, mnrl_mode_coeff, src_depletion), 1
    ...
    extra_args = (mnrl_mode_coeff, src_depletion, part_arr)
    ...
    while solver.status == "running":
        ...
        if (
            ...
            Fn_0 = solid_composition(X_0, P_0, mnrl_mode_coeff, src_depletion)
            melting_step += 1
        # Consider exhausted elements whose normalised concentrations are low
        mask_step[solver.y / cs_0[mask_ode] < 1e-9] = False
        # If the integrator time step collapses,
        # consider that an element is about to exhaust and identify it
...
def non_zero_initial_melt(
    part_arr, part_pot_temp, mant_pot_temp, lab_pres, dTdP_GPa, melt_inputs
):
    step = 0.02
    P = linspace(
    ...
    if part_arr["pressure"][0] > lab_pres - step:
        T = mant_pot_temp + P * dTdP_GPa
    else:
        distrib_temp_over = int((lab_pres - part_arr["pressure"][0]) / step + 1)
        T = (
            part_pot_temp
            + P * dTdP_GPa
            + pad(
